...
-- │                                                        │
-- │ Purpose: Centralized highlight customizations that     │
-- │          override colorscheme defaults for consistent  │
-- │          look across all UI elements.                  │
-- ╰────────────────────────────────────────────────────────╯
...
local function setup_telescope_highlights()
  ...
  -- Preview: slightly lighter than base; Results: slightly darker than base
  local preview_bg = darken_color(base_bg, -0.08) or base_bg
  local results_bg = darken_color(base_bg, 0.08) or base_bg

  local hl_groups = {
    TelescopePreviewNormal = { bg = preview_bg },
    TelescopePreviewBorder = { bg = preview_bg },
    TelescopeResultsNormal = { bg = results_bg },
    TelescopeResultsBorder = { bg = results_bg },
  }
...
local function setup_misc_highlights()
  ...
  -- Floating window background consistent with editor
  set_hl("NormalFloat", { fg = normal_hl.fg, bg = base_bg })
  set_hl("FloatBorder", { fg = base_bg, bg = base_bg })

  -- Create a slightly darker background for active window
  local active_bg = darken_color(base_bg, 0.05) or base_bg

  set_hl("ActiveWindow", { bg = active_bg })
  set_hl("InactiveWindow", { bg = base_bg })
...
local function setup_blink_highlights()
  ...
  -- ── Signature (borderless: background matches editor) ──
...
function M.setup_snacks_indent_highlights()
  ...
  local hint_fg = get_hl("DiagnosticHint").fg or 0x98C379
  local func_fg = get_hl("Function").fg or 0x61AFEF
  local type_fg = get_hl("Type").fg or 0xC678DD
  ...
  local rainbow_hl = {
    ...
    RainbowBlue   = { fg = info_fg },
    RainbowOrange = { fg = constant_fg },
    RainbowGreen  = { fg = hint_fg },
    RainbowViolet = { fg = type_fg },
    RainbowCyan   = { fg = special_fg },
  }
  for name, opts in pairs(rainbow_hl) do
    set_hl(name, opts)
  end
